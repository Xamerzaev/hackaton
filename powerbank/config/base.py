...
class Common(Configuration):
    ...
    SECRET_KEY = env.str(
        "POWERBANK_SECRET_KEY", default="temp"
    )
    ...
    LOGGING = {
        ...
        "formatters": {
            ...
            "sql": {
                "()": "powerbank.loggers.SQLFormatter",
    ...
    SECURE_BROWSER_XSS_FILTER = env.bool(
        "POWERBANK_SECURE_BROWSER_XSS_FILTER",
        default=True,
    )
    SECURE_CONTENT_TYPE_NOSNIFF = env.bool(
        "POWERBANK_SECURE_CONTENT_TYPE_NOSNIFF",
        default=True,
    )
    SESSION_COOKIE_HTTPONLY = env.bool(
        "POWERBANK_SESSION_COOKIE_HTTPONLY",
        default=True,
    )
    SESSION_COOKIE_SECURE = env.bool(
        "POWERBANK_SESSION_COOKIE_SECURE",
        default=True,
    )
    CSRF_COOKIE_SECURE = env.bool(
        "POWERBANK_CSRF_COOKIE_SECURE",
        default=True,
    )
    X_FRAME_OPTIONS = env.str(
        "POWERBANK_X_FRAME_OPTIONS",
        default="SAMEORIGIN",
    )
    SECURE_HSTS_SECONDS = env.int(
        "POWERBANK_SECURE_HSTS_SECONDS",
        default=31536000,
    )  # 1 year
    ...
    ANNON_RATE = env.int(
        "POWERBANK_ANNON_THROTTLE_RATE_PER_MIUTE",
        default=50,
    )
    USER_RATE = env.int(
        "POWERBANK_USER_THROTTLE_RATE_PER_MIUTE",
        default=100,
    )
    ...
    REST_FRAMEWORK = {
        "DEFAULT_AUTHENTICATION_CLASSES": (
            "powerbank.apps.accounts.custom_auth.BearerAuthentication",
        ),
        ...
        "DEFAULT_PAGINATION_CLASS": (
            "powerbank.apps.common.pagination.DynamicPageSizePagination"
        ),
        "PAGE_SIZE": 20,
    ...
    REST_AUTH_SERIALIZERS = {
        ...
        "LOGIN_SERIALIZER": (
            "powerbank.apps.accounts.serializers.CustomLoginSerializer"
        ),
        "TOKEN_SERIALIZER": (
            "powerbank.apps.accounts.serializers.CustomTokenSerializer"
        ),
        "PASSWORD_RESET_SERIALIZER": (
            "powerbank.apps.accounts.serializers"
    ...
    REST_AUTH_REGISTER_SERIALIZERS = {
        "REGISTER_SERIALIZER": "powerbank.apps.accounts.serializers.CustomRegisterSerializer"
    }
    ...
    PASSWORD_RESET_URL = env.str(
        "POWERBANK_PASSWORD_RESET_URL",
        "http://localhost:3000/",
    )

    # Frontend URL to be used for email verification
    EMAIL_VERIFICATION_URL = env.str(
        "POWERBANK_EMAIL_VERIFICATION_URL",
        None
    )

    # Override the account adapter to use the custom one
    ACCOUNT_ADAPTER = "powerbank.apps.accounts.allauth_adapter.CustomAllauthAdapter"
    ...
    LOGOUT_ON_PASSWORD_CHANGE = env.bool(
        "POWERBANK_LOGOUT_ON_PASSWORD_CHANGE",
        default=True,
    )
    ...
    OLD_PASSWORD_FIELD_ENABLED = env.bool(
        "POWERBANK_OLD_PASSWORD_ENABLED",
        default=True,
    )
    ...
    API_PREFIX = env.str(
        "POWERBANK_API_PREFIX", "api"
    )
    API_VERSION = env.str(
        "POWERBANK_API_VERSION", "v1"
    )
    PLATFORM_PREFIX = env.str(
        "POWERBANK_PLATFORM_PREFIX",
        "_platform",
    )
    DOCS_PREFIX = env.str(
        "POWERBANK_DOCS_PREFIX", "docs"
    )

    # Other settings
    COMMUNICATOR_NAME = env.str(
        "POWERBANK_MAIL_COMMUNICATOR_NAME",
        "Admin",
    )

    # For the docs
    CONTACT_EMAIL = env.str(
        "POWERBANK_CONTACT_EMAIL",
        "test@test.com",
    )
    SAMPLE_AUTH_TOKEN = env.str(
        "POWERBANK_SAMPLE_AUTH_TOKEN",
        "sjdhskjh3454343",
    )
    HOSTED_DOMAIN = env.str(
        "POWERBANK_HOSTED_DOMAIN",
        "https://ohuru.tech/",
    )
